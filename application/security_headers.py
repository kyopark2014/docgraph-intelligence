...
here so Google Sign-In and same-origin APIs keep working.
"""
...
def _s3_connect_src_hosts() -> str:
    """Allow browser→S3 presigned PUT/GET used by DocGraph raw uploads.
...
# frame-src includes 'self' so Knowledge/DocGraph modals can iframe HTML
# (without 'self' the graph iframe is blank).
# connect-src includes S3 so DocGraph raw can PUT directly to presigned URLs.
...
def _is_graph_html(scope: Scope) -> bool:
    """Exact Knowledge/DocGraph graph HTML (not /status|/query|/sync)."""
    path = scope.get("path") or "/"
    path = path.rstrip("/") or "/"
    return path in {"/api/graph", "/api/docgraph/graph"}
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        ...
        async def send_with_headers(message: Message) -> None:
            if message["type"] == "http.response.start":
                ...
                if is_graph:
                    # Force framing/CSP suitable for same-origin iframe + unpkg.
