...
export const api = {
  ...
  getWikiStatus: () => request<WikiStatus>("/api/docgraph/status"),
  syncWiki: (full = false, model?: string) => {
    const params = new URLSearchParams();
    if (full) params.set("full", "1");
    if (model?.trim()) params.set("model", model.trim());
    const qs = params.toString();
    return request<WikiStatus>(`/api/docgraph/sync${qs ? `?${qs}` : ""}`, {
      method: "POST",
    });
  },
  setWikiGraphPattern: (pattern: GraphPattern | string) =>
    request<WikiStatus>("/api/docgraph/pattern", {
      method: "PATCH",
      body: JSON.stringify({ pattern }),
    }),
  getWikiSources: () => request<WikiSourcesConfig>("/api/docgraph/sources"),
  putWikiSources: (body: {
    folders: string[];
    foundation_model_parser_enabled?: boolean;
    parallel_processing_enabled?: boolean;
  }) =>
    request<WikiSourcesConfig>("/api/docgraph/sources", {
      method: "PUT",
      body: JSON.stringify(body),
    }),
  ingestWikiUrl: (url: string) =>
    request<WikiUrlIngestResult>("/api/docgraph/urls", {
      method: "POST",
  ...
  browseWikiSources: (path?: string) => {
    ...
    return request<WikiBrowseResult>(`/api/docgraph/browse${q}`);
  },
