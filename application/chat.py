...
def update(userId=None, modelName=None, debugMode=None, guardrailEnabled=None, llmGatewayEnabled=None):
    """Update agent/UI session settings.

    Compatible with:
    - Web UI client: update(modelName=\"...\") or keyword args
    - Agent runtime: update(userId=..., modelName=..., debugMode=..., ...)
    """
    global model_name, model_id, model_type, debug_mode, reasoning_mode
    global models, user_id, guardrail_enabled, llm_gateway_enabled

    # UI shorthand: update("OpenAI GPT 5.5")
    if (
        userId is not None
        and modelName is None
        and debugMode is None
        and isinstance(userId, str)
        and any(k in userId for k in ("OpenAI", "GPT"))
    ):
        modelName = userId
        userId = None
...
def uses_converse_guardrail() -> bool:
    return False  # Input guardrail Converse path is unused for OpenAI GPT
...
def get_persistent_checkpoint_db_path(session_id: str | None = None) -> str:
    """Durable checkpoint path on session storage (/mnt/workspace), per task."""
    if not session_id:
        session_id = _checkpoint_session_id()
    if session_id:
...
# Content block types that should not be replayed across turns (e.g. after
# OpenAI Responses API turns leave type='reasoning').
...
def sanitize_adaptive_thinking_messages(messages: list) -> list:
    ...
    Strips ``thinking`` / ``reasoning`` blocks so later turns stay valid.
    """
    sanitized = []
    for msg in messages:
...
def _build_llm_gateway_chat(profile: dict, max_output_tokens: int):
    """Build chat model via LiteLLM gateway (ChatOpenAI)."""
...
def get_chat():
    ...
    if profile["model_type"] != "openai":
        raise ValueError(f"Unsupported model_type={model_type!r}; only OpenAI GPT models are supported")
...
def append_tool_guidance_to_prompt(system_prompt: str, mcp_servers: list) -> str:
    """Inject MCP tool guidance into the system prompt based on selected servers.

    Mandatory recall lines are prepended (same priority as agent-skills memory),
    and detailed sections are appended.
    """
    if not mcp_servers:
    ...
    if "docgraph" in selected:
        # agent-skills memory와 같이 프롬프트 상단에 필수 호출 문구 배치
        prefixes.append(
            '답변 전에 반드시 recall_docgraph(question=<사용자 질문>)를 1회 이상 호출하세요.\n'
            "DocGraph에 Sync·업로드된 문서에 근거해 답하고, 문서에 없는 사실은 추측하지 마세요."
        )
        extras.append(
            "## DocGraph (문서 코퍼스)\n"
            "- `recall_docgraph(question)`: Sync/업로드 문서·PDF·소스 폴더 그래프를 검색합니다.\n"
            "- 문서·보고서·동향·발주 현황 등 코퍼스와 관련될 수 있는 질문이면 "
            "일반 지식으로 바로 답하지 말고 먼저 호출하세요.\n"
            "- question에는 사용자 질문의 핵심 키워드/주제를 넣으세요."
        )

    if "graph memory" in selected:
        prefixes.append(
            '답변 전에 반드시 recall_graph_memory(question=<사용자 질문>)를 1회 이상 호출하세요.'
        )
        extras.append(
            "## Graph Memory (대화 Knowledge Graph)\n"
            "- `recall_graph_memory`: 과거 대화가 쌓인 Knowledge Graph에서 사용자 맥락을 조회합니다.\n"
            "- skill 지침이 없는 질문이라도 Graph Memory로 사용자 맥락을 먼저 조회한 뒤 답변하세요.\n"
            "- 개인 정보·선호·위치·이전에 공유한 사실이 필요하면 추측하지 마세요."
        )

    parallel_tools: list[str] = []
    if "docgraph" in selected:
        parallel_tools.append("recall_docgraph")
    if "graph memory" in selected:
        parallel_tools.append("recall_graph_memory")
    if "tavily" in selected:
        parallel_tools.append("tavily_web_search")
    if len(parallel_tools) >= 2:
        extras.append(
            f"검색이 필요하면 {', '.join(parallel_tools)}을(를) **병렬로** 호출한 뒤, "
            "결과를 종합해 답하세요. DocGraph는 업로드 문서, Graph Memory는 과거 대화, "
            "웹 검색은 최신/외부 정보용입니다."
        )

    if not prefixes and not extras:
        return system_prompt

    logger.info(f"extra prompt prefixes={prefixes!r} extras={extras!r}")
...
async def create_agent(
    ...
    for server_name in (
        ...
        if params and params.get("transport") == "stdio":
            ...
            env["DOCGRAPH_USER_ID"] = user_id
            params["env"] = env
            logger.info(f"{server_name} MCP DOCGRAPH_USER_ID={user_id}")
...
async def _run_langgraph_agent_impl(
    ...
    if (
        ...
        # MCP 구성이 같아도 프롬프트 가이던스는 매 요청 갱신
        skill_info = skill.get_skill_info(skill_list)
        system_prompt = skill.build_skill_prompt(skill_info)
        system_prompt = append_tool_guidance_to_prompt(system_prompt, mcp_servers)
        agent_config.setdefault("configurable", {})["system_prompt"] = system_prompt
