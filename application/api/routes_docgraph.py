"""Serve DocGraph from per-user ``.session_storage/{user}/docgraph``."""
...
from application.docgraph_jobs import (
    ensure_docgraph_graph_html_current,
    ensure_docgraph_sync,
    get_docgraph_job_status,
    republish_docgraph_graph_html,
)
from application import utils

import logging

logger = logging.getLogger("routes_docgraph")

router = APIRouter(prefix="/api/docgraph", tags=["docgraph"])

# Multipart /api/docgraph/raw still caps at ~ALB body size; prefer /raw/presign for large files.
...
class DocGraphRawPresignRequest(BaseModel):
    file_name: str = Field(..., min_length=1)
    size: int | None = Field(default=None, ge=0)
    content_type: str | None = None


class DocGraphRawCompleteRequest(BaseModel):
    file_name: str = Field(..., min_length=1)
    s3_key: str = Field(..., min_length=1)
    size: int | None = Field(default=None, ge=0)


def _assert_docgraph_presign_size(size: int | None) -> None:
    if size is None:
        return
    if size < 0:
...
def wiki_status(request: Request) -> dict:
    ...
    job = get_docgraph_job_status(user_id)
    ...
    return {
        "docgraph_dir": utils.get_user_wiki_dir(user_id),
        "sources": sources,
        "urls": urls,
...
def put_wiki_sources(body: WikiSourcesPut, request: Request) -> dict:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"DocGraph 소스 저장 실패: {exc}",
        ) from exc
    return {
        "docgraph_dir": utils.get_user_wiki_dir(user_id),
...
def docgraph_raw_presign(request: Request, body: DocGraphRawPresignRequest) -> dict:
    """Return a short-lived S3 PUT URL so the browser can upload past ECS body limits."""
    user_id = require_user_id(request)
    _assert_docgraph_presign_size(body.size)
    name = (body.file_name or "").strip() or "upload.bin"
    try:
        presign = utils.generate_docgraph_raw_presigned_put(name, user_id=user_id)
    except Exception as exc:
        raise HTTPException(
            status_code=500,
...
def docgraph_raw_complete(request: Request, body: DocGraphRawCompleteRequest) -> dict:
    """Confirm a presigned PUT and copy the object into local ``{docgraph}/raw``."""
    user_id = require_user_id(request)
    _assert_docgraph_presign_size(body.size)
    try:
        result = utils.save_docgraph_raw_from_s3(
    ...
    return {
        "docgraph_dir": result["docgraph_dir"],
...
async def upload_wiki_raw_files(
    ...
    """Copy uploaded documents into the user's ``{docgraph}/raw`` for later Sync.
...
def sync_wiki(
    ...
    """Enqueue graphify sync for the user's DocGraph directory."""
    user_id = require_user_id(request)
    utils.ensure_user_wiki_dir(user_id)
    model_name = (model or "").strip() or None
    logger.info(
        "docgraph sync requested user=%s full=%s model=%s",
        user_id,
        full,
        model_name or "(default)",
    )
    job = ensure_docgraph_sync(user_id, full=full, model=model)
    path = wiki_graph_html_path(user_id)
    return {
        "docgraph_dir": utils.get_user_wiki_dir(user_id),
...
def patch_wiki_pattern(body: WikiPatternPatch, request: Request) -> dict:
    ...
    if not wiki_graph_json_path(user_id).is_file():
        raise HTTPException(
            status_code=404,
            detail="DocGraph 그래프가 아직 없습니다. Settings → DocGraph → Sync를 실행하세요.",
        )
    pid = utils.set_wiki_graph_pattern(body.pattern, user_id=user_id)
    try:
        ok = republish_docgraph_graph_html(user_id, pattern=pid)
    except Exception as exc:
        raise HTTPException(
            status_code=500,
            detail=f"DocGraph 그래프 패턴 전환 실패: {exc}",
        ) from exc
    if not ok:
        raise HTTPException(
            status_code=500,
            detail="DocGraph 그래프 HTML을 생성하지 못했습니다.",
        )
    path = wiki_graph_html_path(user_id)
    return {
        "docgraph_dir": utils.get_user_wiki_dir(user_id),
...
def get_wiki_graph(request: Request):
    ...
    if not json_path.is_file():
        ...
        if status in ("queued", "running"):
            detail = "DocGraph 그래프를 동기화하는 중입니다. 잠시 후 다시 열어 주세요."
        elif status == "error":
            detail = f"DocGraph 동기화에 실패했습니다: {job.get('error') or 'unknown error'}"
        else:
            detail = (
                "DocGraph 그래프가 아직 없습니다. Settings → DocGraph → Sync로 "
                "먼저 동기화하세요."
            )
        return HTMLResponse(
            "<!doctype html><html lang='ko'><head><meta charset='UTF-8' />"
            "<title>DocGraph</title></head><body style='"
    ...
    path = ensure_docgraph_graph_html_current(user_id)
...
def query_wiki_graph(body: WikiQueryRequest, request: Request) -> dict:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"docgraph query failed: {exc}",
        ) from exc
