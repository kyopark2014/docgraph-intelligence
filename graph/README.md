...
사이드바 상단 브랜드 **Agent wiki (user)** 를 클릭하면, 로그인한 사용자의 knowledge graph가 새 탭으로 열립니다.

```text
Sidebar "Agent wiki (ksdyb)" 클릭
  → 앱 내 팝업(모달) + iframe → GET /api/graph
  → graph/out/graph_ksdyb.html
```
...
### DocGraph Sync (앱 Settings → DocGraph → Sync)

에이전트 스킬이 아니라 **이 폴더의 고정 파이프라인**입니다 (`docgraph_jobs`가 subprocess로 실행).

| 스크립트 | 역할 |
|----------|------|
| `sync_docgraph.py` | detect → AST → 시맨틱 → cluster → `app-graph.html` |
| `pdf2text.py` | PDF→텍스트 (pdfplumber/pypdf 또는 Foundation Model Parser) |

```bash
cd agent-wiki
python graph/sync_docgraph.py --user ksdyb
python graph/sync_docgraph.py --user ksdyb --full
```
...
# If unset: Bedrock Converse via AWS credentials
...
3. **gateway 없음**: AWS Bedrock Converse
